...
class QSAStateBackend(AttentionBackend):
    """Key-only backend for out-of-band FP16/BF16 QSA side-cache operations."""
...
class QSAKeyStateCache(_QSAStateCache):
    ...
    def __init__(self, *, cache_rope_positions: bool = False, **kwargs) -> None:
        ...
        # Older 1Cat cache binding assigns ``kv_cache`` directly instead of
        # calling bind_kv_cache() for every custom state layer.  Keep the
        # optional field present for the non-MRoPE path in either lifecycle.
    ...
    def bind_kv_cache(self, kv_cache: torch.Tensor) -> None:
        ...
        if kv_cache.dtype != self.dtype or kv_cache.shape[3] != self.head_size:
            raise ValueError(
                "QSA raw cache does not match its packed FP16/BF16 cache spec"
            )
