...
# 1Cat's Qwen3.5 predates the class-level stacked mapper used by current
# upstream vLLM. Keep Qwen4Exp self-contained instead of changing the existing
# Qwen3.5 loading path.
...
class Qwen4ExpSparseMoeBlock(Qwen3NextSparseMoeBlock):
    ...
    def __init__(self, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # Qwen4Exp uses AutoWeightsLoader, which delegates expert subtrees to
        # FusedMoE.load_weights().  1Cat's FusedMoE requires the per-expert
        # checkpoint mapping to be attached to the layer explicitly (current
        # upstream's FusedMoEFactory supplies this implicitly).
        self.experts.expert_mapping = self.experts.make_expert_params_mapping(
            self,
    ...
    def _forward(
        ...
        *,
        ...
        if self.layer_type == "linear_attention":
            # 1Cat's SM70 GDN ABI writes into a caller-owned output tensor.
            attn_out = torch.empty_like(block_input)
            self.linear_attn(hidden_states=block_input, output=attn_out)
        elif self.layer_type == "full_attention":
            if isinstance(self.self_attn, Qwen4ExpQSAAttention):
                attn_out = self.self_attn(
                    hidden_states=block_input,
                    positions=positions,
                )
            else:
                # The pre-Qwen4 Qwen3Next attention ABI in 1Cat also uses a
