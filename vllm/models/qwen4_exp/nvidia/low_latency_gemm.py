...
try:
    ...
    # 1Cat v1.3 predates the Blackwell-only CuteDSL skinny GEMM package.  Keep
    # the standard linear fallback importable on SM70 without pulling that
    # unrelated dependency into the V100 build.
    ...
    class _UnavailableSkinnyGemm:
        @staticmethod
        def is_available() -> bool:
            return False

        def __call__(self, *args: object, **kwargs: object) -> torch.Tensor:
            raise RuntimeError("CuteDSL skinny GEMM is unavailable")

        @staticmethod
        def request_warmup_configs(*args: object, **kwargs: object) -> None:
            return

    shape_dynamic_skinny_gemm = _UnavailableSkinnyGemm()
