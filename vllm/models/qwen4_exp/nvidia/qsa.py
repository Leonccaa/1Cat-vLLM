...
_QSA_DTYPES = (torch.float16, torch.bfloat16)
...
class Qwen4ExpQSAFlashAttentionImpl(FlashAttentionImpl):
    ...
    def forward_qsa(
        ...
        # Current vLLM exposes the logical [B, H, N, K+V] view.  1Cat's
        # pre-layout-refactor allocator still binds FlashAttention's physical
        # [B, 2, N, H, D] tensor, so accept both without copying.
        if kv_cache.ndim == 5 and kv_cache.shape[1] == 2:
            key_cache, value_cache = kv_cache.unbind(dim=1)
        elif kv_cache.ndim == 5 and kv_cache.shape[2] == 2:
            key_cache, value_cache = kv_cache.unbind(dim=2)
        else:
        ...
        if key_cache.dtype != query.dtype or query.dtype not in _QSA_DTYPES:
            raise NotImplementedError("Qwen4Exp QSA requires matching FP16/BF16 Q/K/V")
...
class Qwen4ExpQSAAttention(Qwen3NextAttention, AttentionLayerBase):
    ...
    def __init__(
        ...
        *,
        ...
        if self.kv_cache_torch_dtype not in _QSA_DTYPES:
            raise NotImplementedError("Qwen4Exp QSA requires FP16/BF16 cache storage")
        if self.kv_cache_torch_dtype != model_config.dtype:
            raise NotImplementedError(
                "Qwen4Exp QSA requires cache storage to match the model dtype"
            )
    ...
    def _project_qkv_gate(
        ...
        """Backport Qwen3Next's QKV projection helper for 1Cat.

        Current upstream Qwen3Next provides this method (and an optional fused
        implementation).  1Cat's older class only embeds the eager equivalent
        inside ``forward``; QSA needs the separated post-RoPE tensors.
        """
