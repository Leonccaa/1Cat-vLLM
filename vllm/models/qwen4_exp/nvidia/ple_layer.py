...
def _get_ple_embedding_quant_method(
    ...
    # Flash-Next community checkpoints can be hybrid: ModelOpt NVFP4 for the
    # routed experts while the independently sharded PLE table is official
    # FP8.  In that case the model-level quant config is not Fp8Config, so the
    # checkpoint's explicit PLE dtype is the authoritative selector.
    if ple_embedding_dtype in {"float8_e4m3fn", "float8_e4m3"}:
