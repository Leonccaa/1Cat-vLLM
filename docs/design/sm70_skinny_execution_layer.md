...
- Plain `max_num_seqs=3` had no exact M=3 CUDA Graph. Adding only capture size
  ...
  cliff, not a Skinny gain. The exact-M capture experiment and broader graph
  policy are evidence only and are not part of this Skinny integration; the
  v1.3.0 global CUDA Graph policy remains unchanged. Only B=3 has a measured
  end-to-end speedup, so the result must not be generalized to larger sizes.
...
new classifier or kernel work is planned under this project scope. Raw evidence
...
The 2026-08-12 gate used four PCIe V100 32 GB GPUs with TP4. Raw evidence is
retained privately outside the repository.
