...
Raw evidence for this gate is retained privately outside the repository.

## Historical mixed-ModelOpt evidence (2026-08-13)

The loader changes used for this experiment are outside this Skinny-only
integration. The measurements remain relevant only to the decision to keep
NVFP4 out of default `auto` residency.
...
On the separate experimental loader branch, full VLM loading, TileLang 0.1.10
original prefill, compile/CUDA graph, encoder warmup, and the fixed text request
passed. An explicit image request remained a separate gate.
...
functional smoke, not a full model-quality acceptance. Raw evidence is retained
...
summary are retained privately outside the repository.
