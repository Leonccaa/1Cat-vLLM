...
//   nvcc -O3 -arch=sm_70 -I base -DHARNESS_BASELINE=1 harness.cu -o harness_base
//   nvcc -O3 -arch=sm_70 -I new                       harness.cu -o harness_new
...
static void run_simt(typename Policy::Params params, const half* x, half* y,
  ...
  if (two_rows)
    simt_kernel<Policy, M, KC, 2><<<grid, block, smem, stream>>>(
        params, x, y, n, k, nullptr, nullptr);
  else
    simt_kernel<Policy, M, KC, 1><<<grid, block, smem, stream>>>(
        params, x, y, n, k, nullptr, nullptr);
#else
...
static void run_qpn(typename Policy::Params params, const half* x, half* y,
  ...
  (void)ws;
  (void)k_splits;
  qpn_kernel<Policy, MT><<<dim3(n / 32), dim3(128), 0, stream>>>(params, x, y,
                                                                 n, k, rows);
#else
...
static double check_window(const std::vector<half>& got,
  ...
  for (int row = 0; row < m; ++row) {
    for (int c = 0; c < cols; ++c) {
      ...
      worst = std::max(
          worst, std::fabs((double)__half2float(got[(size_t)row * n + col]) -
                           acc));
    }
  }
...
int main(int argc, char** argv) {
  ...
  for (const Shape& s : shapes) {
    ...
    const int kRotate =
        (int)std::max<size_t>(2, (24u << 20) / std::max<size_t>(1, weight_bytes));
    ...
    for (int m : {1, 2, 3, 4, 8, 16}) {
      ...
      if (m <= 3) {
        ...
        if (m == 1)
          launch = [&](int i) { run_simt<AwqG128SimtPolicy, 1>(sp_at(i), d_x, d_y, n, k, two_rows, 0); };
        else if (m == 2)
          launch = [&](int i) { run_simt<AwqG128SimtPolicy, 2>(sp_at(i), d_x, d_y, n, k, two_rows, 0); };
        else
          launch = [&](int i) { run_simt<AwqG128SimtPolicy, 3>(sp_at(i), d_x, d_y, n, k, two_rows, 0); };
      } else {
        kern = "qpn ";
        splits = choose_splits(k, n);
        if (m <= 8)
          launch = [&](int i) { run_qpn<AwqG128QpnPolicy, 1>(qp_at(i), d_x, d_y, d_ws, n, k, m, splits, 0); };
        else
          launch = [&](int i) { run_qpn<AwqG128QpnPolicy, 2>(qp_at(i), d_x, d_y, d_ws, n, k, m, splits, 0); };
      }

      launch(0);
      CUDA_OK(cudaDeviceSynchronize());
      CUDA_OK(cudaGetLastError());
      CUDA_OK(cudaMemcpy(got.data(), d_y, got.size() * 2,
                         cudaMemcpyDeviceToHost));
      const int cols = std::min(kCheckCols, n);
      const double err =
          std::max(check_window(got, nib, scales, biases, x, m, n, k, groups, 0,
                                cols),
                   check_window(got, nib, scales, biases, x, m, n, k, groups,
                                n - cols, cols));
    ...
    for (int r = 0; r < kRotate; ++r) {
      cudaFree(d_codes[r]); cudaFree(d_qcodes[r]); cudaFree(d_scales[r]);
      cudaFree(d_biases[r]); cudaFree(d_qscales[r]); cudaFree(d_qbiases[r]);
    }
